...
def verify_formulas(max_n=20):
    """
    Verify the identities for small parameters under the assumption t = n - m - 1.
    We check:
    ...
    for n in range(3, max_n + 1):
        ...
        for m in range(0, n):
            ...
            for j in range(0, m + 1):
                ...
                if lhs1 != rhs1:
                    all_ok = False
                    print("Mismatch in T_{j,t}:")
                    print(f"  n={n}, m={m}, t={t}, j={j}")
                    print(f"  LHS = {lhs1}")
                    print(f"  RHS = {rhs1}")
                    print()

            # Check T_{j,t-1}, only meaningful when t >= 1
            if t >= 1:
                for j in range(0, m + 1):
                    lhs2 = T_jd(n, t, j, t - 1)
